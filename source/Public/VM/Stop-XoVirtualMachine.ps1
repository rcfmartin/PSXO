function Stop-XoVirtualMachine
{
    <#
    .SYNOPSIS
    ...
    process
    {
        ...
        if ($PSBoundParameters.ContainsKey("ForeceShutdownDelay"))
        {
            $params["foreceShutdownDelay"] = $ForeceShutdownDelay
        }
        ...
        if ($vmExists.PowerState -ine "Running")
        {
            throw "VM is not running. Power State: $($vmExists.PowerState)"
        }

        try
        {
            $body = New-JsonRpcRequest -Method "vm.stop" -Params $params
        ...
        if ($resp)
        {
            $prop = $resp.params.items.psobject.Properties.Value | Where-Object { $_.name_label -ilike "*VM.*shutdown" }
