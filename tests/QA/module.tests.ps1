...
Describe 'Help for module' -Tags 'helpQuality' {
    ...
    It 'Should have a .DESCRIPTION with length greater than 10 characters for <Name>' -ForEach $testCases {
        $functionFile = Get-ChildItem -Path $sourcePath -Recurse -Include "$Name.ps1"
        ...
        $functionHelp.Description.Length | Should -BeGreaterThan 9
    }
    ...
    It 'Should have described all parameters for <Name>' -ForEach $testCases {
        ...
        foreach ($parameter in $parameters)
        {
            ...
            $functionHelp.Parameters.($parameter.ToUpper()).Length | Should -BeGreaterThan 9 -Because ('the parameter {0} must have descriptive description' -f $parameter)
        }
    }
}
